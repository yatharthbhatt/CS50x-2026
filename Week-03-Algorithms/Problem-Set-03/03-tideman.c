...
{
    // If loser has an edge to winner, adding winner->loser creates a direct cycle
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[loser][i])
        {
            // If loser points to i, check if there's a path from i to winner
            if (i == winner || creates_cycle(winner, i))
            {
                return true;
            }
        }
