...
int find_min(void)
{
    int min_votes = voter_count; // Initialize with maximum possible votes
    for (int i = 0; i < candidate_count; i++)
    {
...
bool is_tie(int min)
{
    ...
    // Count non-eliminated candidates and those with minimum votes
    for (int i = 0; i < candidate_count; i++)
    {
        if (!candidates[i].eliminated)
        {
    ...
    // If all active candidates have the minimum votes, it's a tie
    return active_candidates == min_count;
}
